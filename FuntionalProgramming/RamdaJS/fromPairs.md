...
- 키-벨류 쌍의 리스트로 부터 새로운 오브젝트를 생성한다. 만약 어떤 키가 (키-벨류) 쌍에 여러 번 (반복적으로) 등장할 경우 (생성될 오브젝트의 키 중복을 방지하기 위해), 가장 오른쪽의 쌍의 오브젝트 포함된다.
> See also [toPairs](./toPairs.md), [pair](./pair.md).

## 설명

- 키-벨류 쌍의 나열로 부터 오브젝트를 생성한다. 생성된 오브젝트는 프로퍼티로 나열된 쌍의 키와 대응되는 값을 가진다.
- 배열의 원소로 0번째 인덱스가 프로퍼티명이고 1번째 인덱스가 값인 키-값 쌍의 배열을 받는다.
- 배열의 각각의 원소는 오브젝트의 프로퍼티로 변환된다.
- 주어진 배열의 모든 배열 원소가 오브젝트의 프로퍼티로 변환되면 새로 만들어진 오브젝트를 반환한다.
