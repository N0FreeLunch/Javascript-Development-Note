...
- 술어함수를 만족하지 않으면 원소를 구분하고, 술어함수를 만족하면 원소를 연속적으로 나열하기 위한 동작을 구현할 때 사용한다. 예를 들어, 긴 문자열에서 공백을 기준으로 문자열을 분리하고 싶은 경우, 이 함수를 사용하면, 공백으로 구분된 단어를 쭉 나열할 수 있는 것이다.
...
- `list`: 그룹화 할 배열. (배열 대신) 문자열도 받으며, 각각의 문자들로 구성된 리스트로서 다뤄진다.
...
- `→ [[a]]`: 반환 값으로 연속된 두 원소가 주어진 술어 함수의 인자로 할당될 때 거짓을 반환하거나 첫 번째 원소이면 새로운 배열에 넣고, 참을 반환하는 경우 기존 배열에 넣어 나열하므로 모든 원소가 배열인 배열을 반환한다.
