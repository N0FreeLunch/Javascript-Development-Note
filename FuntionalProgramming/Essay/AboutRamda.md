...
이때, JSDoc이나 가능하다면 명시적 타입 선언을 적극 활용하여 함수의 입력과 출력을 명확히 정의하여, 로직을 쉽게 이해할 수 있도록 한다. 타입스크립트의 경우 타입이 복잡할 수 있기 때문에 이를 간단히 할 수 있는 레퍼함수를 작성한다.

또한, 이렇게 조합된 함수는 외부 상태에 의존하지 않는 순수 함수이므로, 복잡한 모킹 없이 입력과 출력만 검증하면 되는 수월한 테스트 환경을 제공한다.

### 레퍼함수

타입스크립트의 함수형 라이브러리는 범용적인 목적으로 설계되었기 때문에 사용자가 정의하기에는 복잡한 타입을 갖는 경우가 많다. 이 때문에 다양한 시그니처를 받도록 타입을 정의하고, 유니온 타입에 대한 처리를 하는 등의 모든 타입을 사용자가 직접 표기할 수는 없다. 따라서 상황에 맞는 단일 시그니처 유니온 타입 중 하나만을 받는 레퍼 함수(타입이 많으면 복잡하니 일부 타입만을 만들도록 한다.)를 만드는 편이 좋다.

타입스크립트의 함수형 라이브러리는 범용적인 목적으로 설계되었기 때문에, 내부적으로 매우 복잡한 제네릭과 유니온 타입을 갖는 경우가 많다.

사용자가 이러한 복잡한 '다형성 타입'을 직접 일일이 정의하거나 처리하는 것은 사실상 불가능하거나 비효율적이다.

따라서, 복잡한 유니온 타입 중 현재 상황(Context)에 필요한 구체적인 단일 시그니처을 갖는 래퍼 함수를 만들어, 타입을 좁혀서(Narrowing) 사용하는 편이 좋다.
...
회사의 업무 시간은 제한적이며, 제한된 시간 안에 많은 일을 해야 한다. 이를 위해서는 필요한 학습의 양과 코드를 수정하고 개선하는 일을 일과 시간 안에 끝낼 수 있어야 한다. 기본적으로 절차지향의 코드를 작성할 수 있어야 하며, 개개인의 역량에 따라 좀 더 나은 추상화를 할 수 있도록 만들어야 여러 사람들이 제한된 시간 안에 문제를 해결할 수 있다. 개인 공부, 야근을 통해서 문제를 해결하고 성장할 수 있을지는 모르지만, 좋은 회사라면 기본적으로 업무 시간안에 학습하고 익히고 적용하고 개발할 수 있는 쉬운 개발 문화를 만들고 싶어 할 것이다.
...
함수형 프로그래밍에서 데이터를 구체적으로 제어하기 위해서 함수를 합성하는 것은, 조합의 근원이 되는 수 많은 함수들을 알고 있어야 가능하며, 이들을 활용해서 어떻게 추상화하는지를 배워야한다.

작은 로직을 설계하는데도 수 많은 라이브러리의 함수를 알아야 하고 이를 사용하기 위한 노력을 해야 한다. 수 많은 함수와 각각의 함수들의 동작을 명확히 이해하고 있어야 하므로 마치 언어처럼 단어와 문법에 해당하는 함수를 알고 있어서 자유자재로 사용할 수 있는 레벨이 되어야 한다. 물론 구조의 특을 제공하는 함수와 유틸리티 함수가 나눠져 있기 때문에 프로그래밍의 문법과 같은 역할을 하는 구조적 툴을 제공하는 함수를 먼저 배우는 것으로 습득 속도를 높일 수 있지만 이를 잘 설명한 문서나 자료를 찾기 어렵다.

## 왜 RamdaJS인가?

위의 설명은 함수형 프로그래밍의 문제점을 지적했다. 그럼에도 불구하고 RamdaJS를 학습할 가치가 있을까? 사실상 실무에 이를 도입하는 것 보다는 ES-Tookit과 같은 유틸리티 라이브러리를 사용하는 것이 좋다. 방대한 문서, 쉬운 설명, 단순한 시그니처, 자바스크립트 기본 함수를 제안하는 등의 장점이 있다.
...
타입스크립트 시대에 타입 추론의 어려움을 주는 DATA-last를 사용하는 Ramda는 시대에 뒤처진 툴이 분명하다. 하지만 클로저와 엘릭서 같은 언어는 별도의 언어를 배워야 하는 반면, 비슷한 추상화를 할 수 있는 Ramda는 자바스크립트 베이스이다. 자바스크립트를 익히면서 함수형 프로그래밍을 배우기에 Ramda만한 것이 없다. 또한 구조적 툴을 제공하는 EffectTS와 같은 라이브러리와 달리, 데이터의 정교한 제어와 조작을 위한 툴을 사용하는 연습을 할 수 있다는 장점이 있다.

LLM의 데이터를 파이프라인으로 가공해야 하는 시대가 왔다. 패턴화되지 않은 문자열을 처리할 때는 타입을 통한 구조적 제어로서의 함수형 프로그래밍은 사용할 수 없다. 문자열의 정교한 제어를 위한 정교한 데이터 제어의 방식을 사용할 수 있는 함수형 프로그래밍을 사용해야 하는데 RamdaJS는 이에 대한 좋은 통찰을 제공한다.
