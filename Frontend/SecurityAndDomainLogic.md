...
강화 학습으로 프론트앤드의 AI 발전은 극대화 될 것이고, 사람들은 원하는 UI를 쉽게 확보 가능할 것이다. 이 때문에 앞으로의 개발자의 방향은 타입스크립트를 통한 프론트 구현에 집중하기 보다는 백앤드나 웹 어셈블리 등의 비즈니스 가치를 방어할 수 있는 쪽으로 이동해야 한다.

## 프론트앤드를 두껍게 만들어야 할 때

타입스크립트는 다방면으로 사용하기 때문에 배워두면 좋지만, 그렇다고 해서 프론트에 너무 힘 줘서 개발하는 것은 필요에 따라 엄격하게 구분을 하는 편이 좋다. 유저에게 동적인 UI/UX를 제공할 필요성이 있을 때만 프레젠테이션 레이어 정도로 만들어야 하고, 프레젠테이션 레이어로 만들어도 UI/UX의 가치가 크기 때문에 프론트앤드에 투자를 해야 할 경우에만 프론트앤드를 두껍게 만들도록 한다.

만약 제공하는 서비스가 프론트앤드에 비즈니스 로직을 좀 넣어도 크게 문제가 되지 않을 정도라면 보안 정도만 생각하는 선에서 프론트를 두껍게 만들고 백앤드는 가벼운 CURD를 사용할 수 있지만, 그렇다면 프론트에 비중을 두기 보다는 백앤드에 비중을 두는게 낫지 않을까?

특히 레일즈 같은 백앤드에서 엑티브레코드에 바로 매핑되는 CURD는 굉장히 만들기 쉬운 반면, 도메인 로직이 비대한 서비스는 커스터마이징이 많이 들어가기 때문에 백앤드는 데이터 저장소의 용도로 만들고 프론트앤드를 두텁게 만드는 경우가 있고, 고 언어로 서비스를 만드는 경우에도 백앤드의 도메인 로직을 작성하기 까다롭기 때문에 도메인 로직이 적은 CURD 용도로 만드는 경우가 많은데 이 방법은 이 글에서 언급한 다양한 이유들 때문에 좋지 않은 경우가 많을 것이다.
...
만약 데이터의 조작 및 변경에 민감한 도메인에서 프론트앤드로 로직을 전가하여 속도를 높인다는 전략을 쓰기 위해서는 프론트앤드에서 변환되는 값의 변화를 주의 깊게 설계를 해야 하고, 크리티컬한 영향을 미치는 값에 대해서는 프론트앤드에서 처리하지 않고 백앤드에 넘겨 주어야 한다. 만약 백앤드를 정말 심플하게 만들고자 한다면, 프론트앤드에서 전달된 데이터들에 대한 검증을 하기 위한 별도의 배치처리 등을 만들어 데이터가 정상적인지 검증을 해야하므로 그냥 비즈니스 로직을 방어하기 위한 API 설계 보다 더 어려워질 수 있다.
